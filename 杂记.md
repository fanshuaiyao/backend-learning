...
1. 混合加密是因为对称加密和非对称加密混合使用，对称加密是公钥-公钥，非对称加密是私钥-公钥
...
- 第一次：客户端发送一个Client Hello，包含了一个随机数、版本号、密码套件列表等

- 第二次：服务端返回一个Server Hello，包含了一个和随机数、支持的版本号、还有从密码列表中选的密码

  服务端会发送 Server Certificate， 包含了数字证书

  服务端发送 Server Hello Done 

  此时客户端拿到数字证书之后开始验证

![image-20250331152422146](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503311524335.png)

- 第三次：客户端发送 Client Key Exchange：包含一个随机数，客户端会用公钥来对这个随机数进行加密

![](C:/Users/DELL/AppData/Roaming/Typora/typora-user-images/image-20250331152726993.png)

- 第四次：

![image-20250331153001490](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503311530537.png)
...
- 进程崩溃不会影响其他进程、线程崩溃可能导致整个进程崩溃
  ...
  - 共享内存：多个进程共享同一块内存
  - 消息队列：由操作系统管理的消息存储队列
  - 信号量：通过计数器来控制多个进程对共享资源的访问
  - 信号：操作系统通过发送异步信号
  - 管道：一个进程写入数据、另一个进程从另外一端读取数据
  - 套接字：使用socket()创建端点
  - 互斥锁
  ...
  - 读写锁：
  - 条件变量：满足条件后唤醒
  - 屏障：多个线程必须都到达某个屏障点后，才能继续执行
  
  ## 进程的状态
  
  就绪状态、运行状态、阻塞状态
  
  ![image-20250331154158351](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503311541452.png)
...
- ReentrantLock支持公平锁和非公平锁，synchornized是不支持公平锁的（ReentrantLock默认使用非公平锁）

（**这个公平和非公平体现在state=0的时候，先来的线程不会去看阻塞队列，直接就会进行CAS操作，而公平锁在于代码中先去看一下队列中有人没有，如果每没人然后在进行CAS**）

- ReentrantLock支持设置超时时间，在一定时间内没有拿到锁，就会放弃等待。synchornized不支持
- ReentrantLock支持多个条件变量，synchornized只支持一个                                                                                                                                                                                                                                                                          
- ReentrantLock支持打断，在某个线程阻塞等待锁的时候，别的线程可以打断他的等待（默认不可打断）
- 相同点：两者都是可以重入的

*ReentrantLock底部维护着**state变量（此变量由volatile修饰，保证可见性和有序性）、双向阻塞队列、Owner线程、一个指头节点的指针和指向尾节点的指针**，实际上他内部有一个内部类Sync类，这个类继承了AQS。*

**AQS：抽象队列同步器。是实现🔒同步的核心类。提供了一些抽象的方法，供子类实现**

- **用state（0：空闲 ， 1：被占用，2：重入）属性来表示资源的状态，使用CAS来进行状态的更新。**
...
- 加锁的时候，通过CAS修改state，将0改为1。

![image-20250325200214366](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503252002584.png)

- 如果发生了竞争，如果有人占用锁了，此时CAS就会失败，去将此线程包装成一个Node节点，通过park加入阻塞队列。
- 如果此时这个节点是第二位，那么继续使用tryAcquire尝试获取锁，如果还是state=1，失败。然后将前驱改为节点的状态改为-1，代表有责任去唤醒后继的节点。此节点成阻塞状态。（在竞争失败后还会发生好几次的重试）
- ![image-20250325200942767](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503252009852.png)

# Volatile

- 对volatile变量的写指令之后会加入写屏障（保证写屏障之前的改动全部同步到主存，不只是只在线程的工作内存当中了。且保证写屏障之前的代码不能重排序到后面）

- 对volatile变量的读指令之前会加入读屏障（保证读取最新的值，防止读屏障后面的指令重排序到这个之前）

  **volatile通过让读写都走主存实现了可见性、有序性**

![](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503252024344.png)

# ConcurrentHashMap

putVal方法

由于是懒加载，所以在第一次put的时候，会先进行判断是否容器初始化了

- 首先是将key值进行hash

- 然后判断是否容器是否初始化，如果初始化了，就创建一个Node节点，使用CAS进行插入(通过（n-1）&hash，与操作优化成位操作)

- 如果没有初始化，开始执行初始化的操作

  - 先查看sizeCtl的值是否小于0，代表有线程在初始化，自己就yelid，回到就绪状态，释放cpu
  - 如果不是，就使用CAS进行交换成-1，表示抢占锁了
  - 抢到做之后还会在做一个检查，容器是否已经初始化，双重检查目的，防止重复初始化
  - 然后将sizeCtl设置为默认容量
  - 整个过程是在while种，自旋的
  ...
  SizeCtl=-N：低16位为参与扩容的线程数

  SizeCtl=0：数组没初始化

  SizeCtl>0：数组已经初始化，sizectl代表扩容的阈值

  ## 保证安全：

  JDK7通过分段锁，粒度太大

  JDK8通过synchronized+CAS，通过CAS置换数据，syn锁来锁数组的第一个节点
...
4. 建立TCP连接：进行三次握手（**TCP协议是面向连接的可靠的基于字节流的传输层通信协议**）
   ...
   **三次握手是保证双方都有接收和发送的能力。**
   ...
   （具体来讲就是**防止历史连接重复初始化**和**同步双方初始化序列号**。当一个客户端的SYN报文阻塞后，冲刺你发一个后，但是旧的比新的更先到，三次握手就可以区拒绝连接，发送RST报文。如果两次的话，服务端收到这个旧的SYN就建立了连接，然后就可以开始发送数据了，导致错误的连接，造成资源浪费。还有一方面就是，我们使用三次握手可以确保双方的初始序列号已经被对方接收，两次握手只能保证发起连接的一方的初始序号被接收了）
