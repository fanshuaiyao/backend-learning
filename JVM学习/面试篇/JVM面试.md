...
* 混合回收 （Mixed GC）：Mixed GC是G1垃圾回收器独有的，也称混合回收，针对年轻代和部分老年代区域的垃圾回收。先执行一次年轻代回收，发现老年代的占有率达到阀值（默认45%）或年轻代被分配大对象时，会触发一次Mixed GC，回收所有年轻代和一部分老年代区（选取的策略是垃圾对象最多的老年代区域，确保释放更多内存空间，即回收价值高的），控制最大暂停时间。
...
G1最大的特征：G1的内存结构并不是连续的，被划分了许多空间。G1不同于其他垃圾回收器，G1是对整个堆进行回收，采用复制算法对每个区域进行回收

![image-20250311172019728](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503111720976.png)

过程：**新对象分配在Eden区，等着Eden分配满了以后，触发了youongGC，将进行扫描Eden区的对象，将活着的对象移动到S区。此时就可以放入新的对象了往eden区中，等再次满了以后，我们就可以再次YoungGC，此时我们将eden和s区的对象放入另外一个S区，S区中的某些可能也要被放到老年区， O区。如果有一个对象太大，在S区来回的移动不划算，我们就把他放入了H区，一个专门存放大对对象的区域。**

- 每个Region区域该多大

默认为2048个每个1M，可以通过jvm参数修改

- 新生代和老年代的比例

![image-20250311173235635](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503111732688.png)

**默认情况下新生代的比例是动态变化的，最小为5%，慢慢增加到60%**，可以通过jvm参数来调整

- 大对象该如何管理

**如果一个对象大于等于分区的一般大小就会被放入H区**

- 除了YGC，还有那种GC

![image-20250311174123914](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503111741078.png)

YGC占用Ende和S区，当Ende满了，触发YGC.。先标记存活的对象，将活的对象移动到S区，然后释放Ende区。然后动态调整Ende区域数量的大小。然后进行判断是否开启了并发标记，如果开启了，并发标记是为了下一步当执行混合GC做准备的。

![image-20250311201506396](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202503112015559.png)
