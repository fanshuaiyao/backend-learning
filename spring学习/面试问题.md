...
![image-20250224211811950](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202502242118202.png)
...
三级缓存是存入一个工厂，再我们放入二级缓存的时候就判断是否需要代理，需要的话就把代理放入二级缓存，这样最后b获取的是a的代理，最后a想要获取代理的时候，也会先去二级缓存找一下，如果有就是用这个代理就行，保证最后是一个a代理。多次调用工厂，可能

## 构造方法注入产生的循环引用问题

因为三级缓存是解决了在依赖注入期间的循环依赖，在bean的生命周期中，构造函数的执行（也就是实例化）是在依赖注入之前执行的，所以就和三级缓存没有什么关联了。

例子： 
![image-20250224212556220](https://shuaiyao85.oss-cn-qingdao.aliyuncs.com/img/202502242125278.png)
